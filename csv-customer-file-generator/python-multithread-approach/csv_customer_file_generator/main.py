...
def main():
    ...
    d_t_generator_init = (end_time_generator_init - start_time_generator_init).seconds
    logger.debug(f"Generator init took {d_t_generator_init}s")
    ...
    d_t_row_generator = (end_time_row_generator - start_time_row_generator).seconds
    logger.debug(f"Generating rows took {d_t_row_generator}s")

    start_time_file_bump = datetime.now()
    Writer.csv_file_writer(command_line_args.target, rows)
    end_time_file_bump = datetime.now()
    d_t_file_bump = (end_time_file_bump - start_time_file_bump).seconds
    logger.debug(f"File Bump took {d_t_file_bump}s")
